...
const Products = () => {
  ...
  // This effect updates the URL and scrolls to top when page changes
  useEffect(() => {
  ...
  // Handlers for filter changes that also reset the page number
  ...
  // Main effect to fetch products from Supabase
  useEffect(() => {
  ...
  const calculateUnitPrice = (product: any, weight: number | 'custom', customWeight?: number) => {
    ...
    const unitPrice = Math.round(pricePerGram * grams * 100) / 100;
    return unitPrice;
  };
  ...
  const handleAddCartDirect = async (product: any, qty: number) => {
    ...
    const unitPrice =
      product.measurement_unit === 'kilograms'
        ? Math.round((product.price / 1000) * (product.default_weight || 500))
        : product.price;
...
// REPLACE IT WITH THIS NEW, CORRECTED BLOCK
useEffect(() => {
  // This effect sets up an observer to animate cards as they scroll into view.
  if (!sectionRef.current || loading) return;

  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        // When a card is intersecting with the viewport
        if (entry.isIntersecting) {
          // Make it visible and add the animation class
  ...
  // Find all product cards that are still invisible and observe them
...
}, [products, loading]); // Re-run this effect when products or loading state changes
...
>
    <div className="container mx-auto px-4">
        ...
        {/* Filter Panel */}
        {showFilters && (
            // NEW: Added flexbox structure and constrained height to fix overflow
            <div className="mt-6 glass rounded-2xl p-4 sm:p-6 flex flex-col shadow-2xl border border-white/30 max-h-[75vh] animate-slide-up">
                ...
                {/* NEW: This div contains all filter options and is now scrollable */}
                <div className="flex-1 overflow-y-auto pr-3 -mr-3 space-y-6 md:space-y-0">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            {/* NEW: Max height and scrolling for category list specifically */}
                            ...
                            <div className="relative space-y-4">
                                ...
                                <Listbox value={sortBy} onChange={handleSortByChange}>
                                    ...
                                    <Listbox.Options className="absolute mt-2 w-full glass rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto text-sm sm:text-base p-2">
                                        {/* NEW: Added styling for active/selected options */}
                ...
                {/* Apply button (NEW: Now fixed to the bottom of the panel) */}
            ...
            {/* Quantity Selection Modal (Updated with Portal) */}
            ...
            {/* Wishlist Message Popup (Updated with Portal) */}
            ...
            {/* Cart Message Popup (Updated with Portal) */}
