...
const AdminEmailTemplates: React.FC = () => {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-dashboard-gradient1">
        <span className="text-xl font-semibold text-white">Loading Templates...</span>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-dashboard-gradient1 text-tamoor-charcoal flex flex-col px-6 md:px-12 py-10">
      ...
      <p className="text-white/80 mb-6">Create, edit, preview and manage your email templates.</p>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEditing(t)} className="text-green-400 hover:text-green-500">
                  <Save className="w-4 h-4" />
                </button>
                <button onClick={() => handleDelete(t.id!)} className="text-red-400 hover:text-red-500">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <span className={`px-2 py-1 rounded-lg text-sm ${t.automation_enabled ? "bg-blue-600" : "bg-gray-500"}`}>Automation</span>
            </div>
            ...
            <div className="mt-4 bg-black/30 p-3 rounded-xl text-white/80 text-sm">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  <Send className="w-4 h-4" /> {sendingTest ? "Sending..." : "Send Test Email"}
                </button>
                <button
                ...
                >
                  <FileText className="w-4 h-4" /> {sendingTest ? "Sending..." : "Send Invoice Email"}
                </button>
                <button
                ...
                >
                  <FileText className="w-4 h-4" /> {sendingTest ? "Sending..." : "Send Confirmation Email"}
                </button>
              </div>
            </div>
          </div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 max-w-3xl w-full flex flex-col gap-4 shadow-lg border border-white/10">
            <h2 className="text-xl font-semibold text-white">{editing.id ? "Edit Template" : "Add New Template"}</h2>
            ...
            <div className="bg-black/30 rounded-xl p-3 text-white/80 text-sm mt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {(PLACEHOLDERS[editing.type] || []).map((ph) => (
                  <span key={ph} className="px-2 py-1 rounded-lg bg-white/20 text-xs">{ph}</span>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-4 mt-2 max-h-[50vh] overflow-y-auto">
              ...
              <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: editing.body }} />
            </div>

            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setEditing(null)} className="px-4 py-2 rounded-xl bg-gray-500 hover:bg-gray-600 text-white">Cancel</button>
              <button onClick={handleSave} className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white" disabled={saving}>{saving ? "Saving..." : "Save Template"}</button>
            </div>
          </div>
        </div>
      )}
