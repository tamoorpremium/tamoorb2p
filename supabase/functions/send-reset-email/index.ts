...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data, error } = await supabase.auth.admin.generateLink({
      ...
      email,
      options: {
        redirectTo: "http://localhost:3000/reset-password", // your frontend page
      },
    });
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "onboarding@resend.dev", // ✅ safe sandbox sender
        to: [email],
    ...
    return new Response(JSON.stringify({ success: true, resetUrl }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (err) {
